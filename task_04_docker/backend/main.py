...
class RecordIn(BaseModel):
    ...
    def validate_datetime(cls, v):
        """Validate that datetime is parseable."""
        try:
            pd.to_datetime(v)
        except Exception as exc:
            raise ValueError(
                "Invalide datetime. Enter a datetime string in "
                "such format '2024-01-01 00:00:00'."
            ) from exc
        return v
    ...
    def validate_energy_europe(cls, v):
        """Validate that energy_europe is non-negative."""
        if v < 0:
            raise ValueError(
                'Invalide energy_europe. '
    ...
    def validate_energy_asia(cls, v):
        """Validate that energy_asia is non-negative."""
        if v < 0:
            raise ValueError(
                'Invalide energy_asia. Energy consumption cannot be negative.'
            )
        return v
    ...
    def validate_price_europe(cls, v):
        """Validate that price_europe is non-negative."""
        if v < 0:
            raise ValueError(
                'Invalide price_europe. Price cannot be negative.'
            )
        return v
    ...
    def validate_price_asia(cls, v):
        """Validate that price_asia is non-negative."""
        if v < 0:
...
def load_data() -> pd.DataFrame:
    """Load records from the CSV file and return as a DataFrame."""
    try:
        ...
        return pd.DataFrame(columns=['id', 'datetime', 'energy_europe',
                                     'energy_asia', 'price_europe',
                                     'price_asia'])
...
def get_records():
    """Fetch and return all records from the CSV file."""
    try:
        df = load_data()
...
def add_record(record: RecordIn):
    """Add a new record to the CSV file and return it."""
    try:
        df = load_data()
        new_id = next_id(df)
...
def delete_record(record_id: int):
    """Delete a record by ID from the CSV file."""
    try:
        df = load_data()
